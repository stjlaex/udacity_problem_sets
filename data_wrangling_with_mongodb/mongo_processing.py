        ...
        for line in reader:
            ...
            for field in process_fields:
                if line[field] == 'NULL':
                    ...
                    data_row[FIELDS[field]] = line[field].strip()


            if data_row['label'].find('('):
                data_row['label'] = data_row['label'][:data_row['label'].find('(')].strip()

            if data_row['name'] == None or re.match(data_row['name'], '[^a-zA-Z0-9 :]'):
                data_row['name'] = data_row['label']

            if data_row['synonym'] != None:
                data_row['synonym'] = parse_array(data_row['synonym'])


            data_row['classification'] = {
                    "kingdom": data_row.pop('kingdom'),
                    "family": data_row.pop('family'),
                    "order": data_row.pop('order'),
                    "phylum": data_row.pop('phylum'),
                    "genus": data_row.pop('genus'),
                    "class": data_row.pop('class')
            }
...
def test():
    ...
    assert data[48]["label"] == "Hydrachnidia"
